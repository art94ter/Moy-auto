(function(){
  ...
  const css = `
    ...
    #homeNav{display:flex!important;position:fixed!important;left:12px!important;right:12px!important;bottom:calc(12px + env(safe-area-inset-bottom))!important;width:auto!important;height:74px!important;margin:0!important;padding:5px!important;gap:5px!important;z-index:9999!important;border-radius:25px!important;border:1px solid rgba(255,255,255,.20)!important;background:linear-gradient(145deg,rgba(37,43,52,.78),rgba(8,11,16,.86))!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.18),inset 0 -1px 0 rgba(0,0,0,.45),0 22px 60px rgba(0,0,0,.55)!important;backdrop-filter:blur(30px) saturate(165%)!important;-webkit-backdrop-filter:blur(30px) saturate(165%)!important}
    #homeNav button{color:#c9c4bb!important;background:linear-gradient(145deg,rgba(255,255,255,.075),rgba(255,255,255,.018))!important;border:1px solid rgba(255,255,255,.08)!important;border-radius:21px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.08)!important;text-shadow:none!important}
    ...
    .quick-grid,.stats-grid{gap:12px!important}.quick,.stat-card{color:var(--text)!important;border:1px solid rgba(244,207,136,.43)!important;background:linear-gradient(145deg,rgba(255,255,255,.105),rgba(255,255,255,.028))!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.13),inset 0 -1px 0 rgba(0,0,0,.25),0 18px 44px rgba(0,0,0,.30)!important;backdrop-filter:blur(28px) saturate(170%)!important;-webkit-backdrop-filter:blur(28px) saturate(170%)!important}.quick{height:168px!important;padding:18px!important}.quick strong,.stat-card b{color:#f7f4ee!important;text-shadow:none!important}.quick small,.stat-card span{color:#c5beb4!important}
    /* REAL DOM arrows — not pseudo-elements, so they cannot disappear behind button styles. */
    .quick .moy-arrow{position:absolute!important;right:14px!important;bottom:10px!important;display:grid!important;place-items:center!important;width:30px!important;height:30px!important;border-radius:50%!important;color:#f4cf88!important;-webkit-text-fill-color:#f4cf88!important;background:rgba(244,207,136,.10)!important;border:1px solid rgba(244,207,136,.42)!important;font-size:22px!important;font-weight:400!important;line-height:1!important;z-index:20!important;opacity:1!important;visibility:visible!important}.quick:after{display:none!important}
    .moy-back{display:flex!important;align-items:center!important;gap:8px!important;width:max-content!important;margin:0 0 14px!important;padding:9px 14px!important;border-radius:14px!important;border:1px solid rgba(244,207,136,.42)!important;background:linear-gradient(145deg,rgba(255,255,255,.09),rgba(255,255,255,.025))!important;color:#f4cf88!important;-webkit-text-fill-color:#f4cf88!important;font-size:13px!important;font-weight:750!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.12),0 10px 25px rgba(0,0,0,.25)!important;backdrop-filter:blur(20px)!important;-webkit-backdrop-filter:blur(20px)!important}
    .moy-back span{font-size:20px!important;line-height:14px!important}
    ...
    .event-empty,.card{background:linear-gradient(145deg,rgba(255,255,255,.075),rgba(255,255,255,.025))!important;border:1px solid rgba(255,255,255,.16)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 18px 48px rgba(0,0,0,.28)!important;backdrop-filter:blur(24px) saturate(160%)!important;-webkit-backdrop-filter:blur(24px) saturate(160%)!important}.events-head button{color:var(--gold2)!important;background:rgba(216,177,109,.045)!important;border-color:rgba(244,207,136,.45)!important}
    .quick *, .stat-card *, .event-empty *, .card *{text-shadow:none}.quick strong,.stat-card b{color:#f7f4ee!important}.kicker,.nav-icon,.event-icon,.section-head h2:after,.events-head h2:after{color:var(--gold2)!important}
  ...
  function install(){let s=document.getElementById('my-auto-premium-glass-v4');if(!s){s=document.createElement('style');s.id='my-auto-premium-glass-v4';document.head.appendChild(s)}s.textContent=css}
  ...
  function addQuickArrows(){
    document.querySelectorAll('.quick').forEach(card=>{
      if(!card.querySelector('.moy-arrow')){
        const a=document.createElement('span');a.className='moy-arrow';a.setAttribute('aria-hidden','true');a.textContent='›';card.appendChild(a)
      }
    })
  }
  ...
  function addBackButtons(){
    document.querySelectorAll('.view').forEach(view=>{
      ...
      if(view.querySelector(':scope > .moy-back'))return;
      const b=document.createElement('button');b.type='button';b.className='moy-back';b.innerHTML='<span>‹</span> Назад';
      b.addEventListener('click',()=>{
        const home=document.querySelector('.view#home');
        if(typeof window.showTab==='function')window.showTab('home');
        else if(home){document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));home.classList.add('active')}
        syncHomeNav()
      });
  ...
  function run(){install();cleanDuplicateNav();syncHomeNav();addQuickArrows();addBackButtons();bindSwipe()}
